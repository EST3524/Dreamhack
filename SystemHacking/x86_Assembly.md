...
mov 명령문에서 dst에 주소를 사용하려면 크기 지정자가 붙어야 하는 것과 달리, lea 명령문에서 크기 지정자가 붙지 않는 이유는 mov는 src에서 어느정도 크기 만큼 대입해야하는지 결정해야하는 것과 달리 lea는 유효 주소를 계산해서 저장하면 되기 때문에 크기 지정자가 붙을 필요 없이 자동으로 저장된다.
